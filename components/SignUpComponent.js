...
const SignUpComponent = () => {
  ...
  const [batches, setBatches] = useState(['2019', '2020', '2021', '2022', '2023']);
  useEffect(() => {
    fetchBatchesAPI()
      .then(result => {
        if (result.error) {
  ...
  const isValid = () => {
    if (values.name.trim().length === 0) {
      ...
      return false;
    } else if (!values.regNo.match(/^\d{5}$/)) {
      setFormErrors({
        ...formErrors,
        regNo: {
          show: true,
    ...
    if (isValid()) {
      ...
      const user = {
        ...
        student_details: {
          isEligible: 'Pending',
          batch: values.batch,
          regNo: `${values.regNo}/${values.department}/${values.batch}`
        }
      };
      signup(user)
        .then(data => {
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={6} sm={4}>
              <FormControl fullWidth error={formErrors.department.show} variant="outlined"
                ...
                >
                  <MenuItem value='BCS'>BCS (CB)</MenuItem>
                  <MenuItem value='BCN'>BCN (CA)</MenuItem>
                  <MenuItem value='BCM'>BCM (CD)</MenuItem>
                </Select>
